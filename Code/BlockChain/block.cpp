...
void getHash(BlockNode* b)//交易单转hash
{
    ...
    while (pos<5)//交易单不空
    {
        ...
        BlockTest_CTX ctx;
        blockTest_init(&ctx);
        blockTest_update(&ctx,(BYTE*)e.launch, strlen(e.launch));
...
void calHash(BlockNode* b)//计算区块Hash
{
    ...
    {
        BYTE c1[BlockTest_BLOCK_SIZE];
        DeQueueHash(b->hashSum, c1); //取第一个hash
        BYTE c2[BlockTest_BLOCK_SIZE];
        DeQueueHash(b->hashSum, c2); //取第二个hash

        int len = BlockTest_BLOCK_SIZE + BlockTest_BLOCK_SIZE;
        BYTE bs[64];
        //拼接两个哈希值
    ...
    BYTE bbs[BlockTest_BLOCK_SIZE];
  //  printf("hash队列长度放入前=%d\n", b->hashSum->size);
    DeQueueHash(b->hashSum, bbs);
    memcpy(b->hash,bbs, BlockTest_BLOCK_SIZE);
}
